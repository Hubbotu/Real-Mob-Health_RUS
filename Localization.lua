...
--		Numbers
	...
	ruRU={--	Translation by Хоргул-Гордунни aka Hubbotu
--		Commands
		Command_NoUnitData="Цель не найдена.";
		Command_DeletedUnitFormat="Удалить %2$s(%1$s)";--	format(CreatureKey,UnitName)
		Command_HealthWiped="Health data wiped.";

--		Options Panel
		Options_DynamicList={--	Button Text,Tooltip Title,Description
--					Tooltip Title=false to use Button Text
			ShowStatusBarTextAdditions={"Показать текст рамки цели",false,"Создает текст для рамок цели."};
			ModifyHealthBarText={"Изменить стандартный фрейм здоровья",false,"Показывает реальные значения здоровья для монстров в стандартных фреймах."};
			ShowTooltipText={"Показать подсказку",false,"Показывает, присутствует ли здоровье моба в кэше здоровья."};
			ShowTooltipHealthText={"Показать подсказку о здоровье",false,"Показывает здоровье на панели состояния игровыми подсказками"};
			ShowNamePlateHealthText={"Отображаеть здоровье возле имени",false,"Показывает здоровье на левой стороне панели."};

			EnablePeerCache={"Включить","Включить общий доступ к кешу","Переключает получение данных о здоровье от других.\n|cffffff00Примечание. Этот аддон будет по-прежнему отвечать на запросы и сообщать об обновлениях работоспособности.|r"};
		};
		Options_SetByAddOn="|cffff0000Эта опция была отменена другим аддоном.|r";

--		Options Cache Panels
		Options_HealthCachePanel={"Кеш здоровья","Хранит данные о здоровье от встреченных монстров."};
		Options_PeerCachePanel={"Поделиться кешем","Хранит данные о здоровье, сообщенные другими игроками."};
...
--		Options Clear Cache Buttons
		...
		Options_ClearPeerCacheTTText="Очистить кэшированные данные";
		Options_ClearCacheWarning="|cffff0000Предупреждение! Все данные в этом кэше будут потеряны навсегда.|r";

--		UITweaks Tooltip Lines
		UITweaksTooltip_Blacklisted="Чёрный список монстров";
		UITweaksTooltip_HealthRecorded="Записанные данные о здоровье";
		UITweaksTooltip_HealthOverride="Данные о здоровье переопределены";
		UITweaksTooltip_NoData="Недостающие данные о здоровье";
